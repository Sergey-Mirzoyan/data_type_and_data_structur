...
template<class T>
vector<T> BinaryTree<T>::AcrossWalkVector(void)
{
    Deque<Node<T> *> queue;
    vector<T> answer;
    if (root == NULL)
...
void BinaryTree<T>::visualize(bool full, string filename, string pic)
{
    ...
    out << "}";
    out.close();
    string script1 = " dot -Tpng " + filename + " -o " + pic;//"/usr/bin/dot -Tpng " + filename + " -o " + pic;
    system(script1.data());
...
void BinaryTree<T>::visualizeColorRepeating(bool full, string filename, string pic)
{
    ...
    out << "}";
    out.close();
    string script1 = " dot -Tpng " + filename + " -o " + pic;
    system(script1.data());
    //string script2 = "/usr/bin/xdg-open " + pic;
    //system(script2.data());
}
...
template<class T>
void BinaryTree<T>::removeDublicates(void)
{
    vector<T> v = AcrossWalkVector();
    vector<T> equalentrys;
    vector<int> counts;
...
template<class T>
unsigned long BinaryTree<T>::memory(void)
{
    vector<T> v = AcrossWalkVector();
