...
int main(void)
{
    ...
    struct queue *q_1 = malloc(sizeof(queue));
    struct queue *q_2;
        ...
        if (q_2)
        {
            ...
            while (cn_out_1 != OUT_MAX)
            {
                ...
                if (eq_1 == 0) // T1 = T2
                {
                    if (oa_do == 0)
                    {
                        if (push(q_1, get_rand_range_int(min, max)) == 0)
                        {
                            cn_in_1++;
                        }
                        else
                            printf("Очередь 1 переполнена\n\n");
                        
                        if (push(q_2, get_rand_range_int(min, max)) == 0)
                        {
                            cn_in_2++;
                        }
                        else
                            printf("Очередь 2 переполнена\n\n");
