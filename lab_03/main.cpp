...
bool IsPalindrome(STACK<T> &stack)
{
	...
	{
		if (temp_stack.Pop(false) != stack[i])
		{
			return false;
		}
	}
...
void UserCommands(STACK<T>& stack)
{
	...
	while (true)
	{
		...
		std::cout << std::endl << "Work with stack {0, 1, 2, 3} {exit, push, pop, polindrome}: " << std::endl;
		std::cin >> command;
...
int main(void) {
	try
	{
		...
		std::cout << std::endl << "Input string(double-click Enter): ";
